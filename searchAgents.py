...
def cornersHeuristic(state, problem):
  ...
  import itertools
  current_pos, visited_corners = state
  unvisited = [problem.corners[i] for i in range(len(problem.corners)) if not visited_corners[i]]
  
  if not unvisited:
    return 0

  # Shortest Manhattan path visiting all remaining corners
  min_cost = float('inf')
  for perm in itertools.permutations(unvisited):
    cost = util.manhattanDistance(current_pos, perm[0])
